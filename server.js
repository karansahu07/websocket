...
const io = socketIo(server, {
  cors: {
    origin: 'http://localhost:8100', // Allow Ionic dev server
    methods: ['GET', 'POST'],
  },
});
...
const allowedUserIds = new Set(); // Limit to 2 users max
...
//     console.log('Received message:', msg);
//     const senderId = msg.user;
//     const receiverId = [...allowedUserIds].find(id => id !== senderId);
...
//     if (receiverSocketId) io.to(receiverSocketId).emit('message', messagePayload);
//     if (senderSocketId) io.to(senderSocketId).emit('message', messagePayload);
//   });
